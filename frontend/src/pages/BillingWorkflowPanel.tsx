...
export default function BillingWorkflowPanel({ onCreated }: { onCreated: (invoice: InvoiceResponse) => void }) {
  ...
  async function load(event?: FormEvent<HTMLFormElement>) {
    ...
    if (!uuid.test(appointmentId)) { setError('Nhập Appointment UUID hợp lệ.'); return; }
    setBusy(true);
    try {
      ...
      if (booking.id !== appointmentId || services.appointmentId !== appointmentId ||
          ...
          !Array.isArray(catalogRows)) throw new Error('Dữ liệu tính phí không khớp lịch hẹn');
  ...
  async function update(action: () => Promise<PerformedServicesResponse | LabBillableItemsResponse>, kind: 'service' | 'lab', label: string) {
    ...
    try {
      ...
      if (result.appointmentId !== appointment.id) throw new Error('Backend returned a different appointment');
      if (kind === 'service') setPerformed(result as PerformedServicesResponse);
      else setLaboratory(result as LabBillableItemsResponse);
      setNotice(label);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Thao tác không được backend xác nhận'); }
    finally { setBusy(false); }
  }
  ...
  async function issue() {
    ...
    try {
      ...
      if (issued.appointmentId !== appointment.id || issued.status !== 'UNPAID'
        ...
        throw new Error('Backend chưa xác nhận hóa đơn có đơn giá và trạng thái hợp lệ');
      }
      onCreated(issued);
      reset(); setAppointmentId('');
      setNotice(`Hóa đơn ${issued.id} đã được backend tạo ở trạng thái CHƯA THANH TOÁN.`);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Không thể phát hành hóa đơn'); }
    finally { setBusy(false); }
  }

  return <section className="panel settings-form" aria-label="Quy trình xuất hóa đơn có kiểm chứng">
    <h3>Xuất hóa đơn từ dữ liệu thực tế</h3>
    <p>Thứ tự: hoàn tất khám → ghi dịch vụ → chốt dịch vụ và xét nghiệm → phát hành hóa đơn → xác nhận thu tiền mặt nếu đã nhận tiền.</p>
    <form onSubmit={(event) => void load(event)}>
      <label>Appointment UUID đã hoàn thành<input required value={appointmentId}
        onChange={(event) => { reset(); setAppointmentId(event.target.value.trim()); }} /></label>
      <button type="submit" disabled={busy || !appointmentId}>Kiểm tra dữ liệu tính phí</button>
    </form>
    ...
    {appointment && performed && laboratory && <>
      ...
      {!performed.finalized && <form onSubmit={(event) => void add(event)}>
        <label>Dịch vụ từ Catalog<select required value={serviceId} onChange={(event) => setServiceId(event.target.value)}>
          <option value="">{catalog?.length ? 'Chọn dịch vụ' : 'Catalog chưa có dịch vụ đang hoạt động'}</option>
      ...
      <p>Đơn giá và revision được Billing Service kiểm tra lại khi phát hành. Không có thanh toán trực tuyến.</p>
    </>}
  </section>;
}