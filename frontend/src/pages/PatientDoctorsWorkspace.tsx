...
/** Only active doctor profiles and role-scoped name-only account data. */
export default function PatientDoctorsWorkspace({ onAppointments }: { onAppointments?: () => void }) {
  ...
  const filtered = (doctors ?? []).filter((item) => `${item.id} ${item.fullName ?? ''} ${item.specialtyName ?? ''}`.toLocaleLowerCase('vi-VN').includes(search.trim().toLocaleLowerCase('vi-VN')));
  ...
  return <div className="patient-portal" aria-label="Danh sách bác sĩ bệnh nhân"><PageHeader title="Bác sĩ phòng khám" subtitle="Tìm chuyên khoa và tham khảo lịch làm việc trước khi đặt lịch"
    ...
    <section className="panel patient-panel"><div className="patient-section-head"><div><span>DANH BẠ BÁC SĨ</span><h3>Tra cứu theo tên hoặc chuyên khoa</h3><p>{doctors ? `${filtered.length}/${doctors.length} bác sĩ phù hợp` : 'Chỉ tải bác sĩ đang hoạt động từ API.'}</p></div></div>
      <label className="patient-search"><Search size={18} aria-hidden="true" /><span className="sr-only">Tìm bác sĩ</span><input type="search" aria-label="Tìm bác sĩ" placeholder="Tên bác sĩ, mã hoặc chuyên khoa..." value={search} onChange={(event) => setSearch(event.target.value)} /></label>
      ...
      <div className="patient-directory-grid" style={{ marginTop: '1rem' }}>{filtered.map((doctor) => <article className="patient-doctor-card" key={doctor.id}>
        <div className="patient-doctor-card-header"><span><Stethoscope size={21} /></span><div><strong>{doctorName(doctor)}</strong><small>{doctor.specialtyName || 'Chưa cập nhật chuyên khoa'} · #{shortId(doctor.id)}</small></div></div>
        <p>Giá khám tham khảo: {doctor.consultationFee == null ? 'Chưa cung cấp' : formatVnd(doctor.consultationFee)}</p>
    ...
    {selected && <section className="panel patient-panel" aria-label="Lịch làm việc bác sĩ"><div className="patient-section-head"><div><span>CA LÀM VIỆC CÔNG KHAI</span><h3>{doctorName(selected)}</h3><p>{selected.specialtyName || 'Chưa cập nhật chuyên khoa'}</p></div><CalendarClock size={22} /></div>
