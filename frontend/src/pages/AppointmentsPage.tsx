...
import { cancelAppointment, completeAppointment, confirmAppointment, createAppointment, getAppointment } from '../api/clinic';
import Alert from '../components/Alert';
import Avatar from '../components/Avatar';
import Badge from '../components/Badge';
import PageHeader from '../components/PageHeader';
import type { AppointmentResponse, CreateAppointmentRequest } from '../types/domain';
...
function PersonalAppointmentsPage({ appointments, role, error, loading, onRefresh }: AppointmentsPageProps) {
  ...
  async function handleBooking(event: FormEvent<HTMLFormElement>) {
    ...
    const succeeded = await runAction(() => createAppointment({ ...booking, doctorId: booking.doctorId.trim(), reason: booking.reason.trim() }), 'Appointment requested.');
  ...
  return (
    <>
      ...
      {isPatient && integrations.appointmentOwnership && showBooking && <form className="panel settings-form" onSubmit={(event) => void handleBooking(event)}>
        ...
        <p>Doctor directory is not available yet. Enter the doctor's UUID supplied by the clinic.</p>
        <label>Doctor UUID<input value={booking.doctorId} required onChange={(event) => setBooking({ ...booking, doctorId: event.target.value })} /></label>
        ...
        <button type="submit" disabled={busy}>Submit booking</button>
      </form>}
      ...
      <section className="split-page split-appointments">
        ...
        {selected && <article className="panel detail-panel">
          ...
          <div className="detail-actions">
            ...
            {role === 'DOCTOR' && integrations.appointmentOwnership && selected.status === 'CONFIRMED' && <button type="button" className="soft-button success" disabled={busy} onClick={() => void runAction(() => completeAppointment(selected.id), 'Appointment completed.')}>Complete Appointment</button>}
          </div>
        </article>}
      </section>
    </>
