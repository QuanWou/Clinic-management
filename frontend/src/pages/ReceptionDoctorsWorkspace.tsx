  ...
  return <div className="reception-doctors" aria-label="Danh sách bác sĩ dành cho lễ tân">
    ...
    <section className="reception-doctors-hero" aria-label="Tổng quan bác sĩ lễ tân">
      <div><span className="reception-doctors-kicker"><ShieldCheck size={15} aria-hidden="true" /> KHÔNG GIAN LỄ TÂN · TRA CỨU</span>
        ...
        <span className="reception-doctors-hero-tag"><CheckCircle2 size={15} aria-hidden="true" /> Danh sách bác sĩ từ API · Không có dữ liệu mẫu</span>
      </div>
    ...
    <section className="reception-doctors-metrics" aria-label="Thống kê danh sách đã tải">
      <article><span className="reception-doctors-metric-icon teal"><UsersRound size={21} aria-hidden="true" /></span><strong>{doctors ? doctors.length.toLocaleString('vi-VN') : '—'}</strong><h3>Bác sĩ đang hoạt động</h3><p>Trong danh sách API trả về</p></article>
    ...
    <div className="reception-doctors-layout">
      <section className="panel reception-doctors-directory" aria-label="Tra cứu bác sĩ">
        <div className="reception-doctors-section-head"><div><span>ĐỘI NGŨ KHÁM BỆNH</span><h3>Danh sách bác sĩ</h3><p>Chỉ gồm bác sĩ đang hoạt động trong API dành cho nhân viên đã đăng nhập.</p></div><span className="reception-doctors-scope"><ShieldCheck size={14} aria-hidden="true" /> Chỉ tra cứu</span></div>
        <div className="reception-doctors-filters">
          <label><Search size={17} aria-hidden="true" /><span className="reception-doctors-sr-only">Tìm bác sĩ</span>
            <input type="search" aria-label="Tìm bác sĩ" placeholder="Mã bác sĩ hoặc tên chuyên khoa..." value={search} onChange={(event) => changeFilters(event.target.value, specialtyId)} />
        ...
        {!loading && doctors && <>
          ...
          {visible.length > 0 && <div className="reception-doctors-list">{visible.map((doctor) =>
            <button type="button" key={doctor.id} className={`reception-doctors-row${selectedId === doctor.id ? ' is-selected' : ''}`} aria-pressed={selectedId === doctor.id}
              ...
              <span className="reception-doctors-row-copy"><strong>Bác sĩ #{shortId(doctor.id)}</strong><small><Stethoscope size={13} aria-hidden="true" /> {doctor.specialtyName || 'Chưa cập nhật chuyên khoa'}</small><span>Mã hồ sơ: {doctor.id}</span></span>
              <span className="reception-doctors-row-action">Chi tiết <ArrowRight size={14} aria-hidden="true" /></span>
            </button>)}
          </div>}
          {!filtered.length && <div className="reception-doctors-empty"><Search size={27} aria-hidden="true" /><strong>{doctors.length ? 'Không tìm thấy bác sĩ phù hợp' : 'Chưa có bác sĩ đang hoạt động'}</strong><p>{doctors.length ? 'Thử thay đổi bộ lọc hoặc từ khóa tìm kiếm.' : 'API hiện chưa trả về hồ sơ bác sĩ hoạt động nào.'}</p></div>}
        ...
        {!loading && !doctors && <p className="reception-doctors-loading">Danh sách bác sĩ chưa khả dụng. Nhấn Thử lại để tải từ API.</p>}
      </section>

      <aside className="panel reception-doctors-detail" aria-label="Chi tiết bác sĩ và lịch làm việc">
        <div className="reception-doctors-section-head"><div><span>THÔNG TIN TRA CỨU</span><h3>{selected ? `Bác sĩ #${shortId(selected.id)}` : 'Hồ sơ & lịch làm việc'}</h3><p>Dữ liệu do Doctor Service cung cấp.</p></div>
        ...
        {!selected ? <div className="reception-doctors-detail-placeholder"><BookOpen size={36} aria-hidden="true" /><strong>Chọn bác sĩ để xem chi tiết</strong><p>Xem chuyên khoa, mã hồ sơ và lịch làm việc trong tuần trước khi đến bước đặt lịch.</p></div> : <>
          <div className="reception-doctors-detail-person"><span className="reception-doctors-avatar large"><UserRound size={30} aria-hidden="true" /></span><div><strong>Bác sĩ #{shortId(selected.id)}</strong><span><Stethoscope size={15} aria-hidden="true" /> {selected.specialtyName || 'Chưa cập nhật chuyên khoa'}</span></div></div>
          <dl className="reception-doctors-facts"><div><dt><IdCard size={15} aria-hidden="true" /> Mã hồ sơ bác sĩ</dt><dd>{selected.id}</dd></div>
            ...
            <div><dt><ClipboardList size={15} aria-hidden="true" /> Phí khám tham khảo*</dt><dd>{selected.consultationFee == null ? 'Chưa cập nhật' : formatMoney(selected.consultationFee)}</dd></div></dl>
          ...
          <div className="reception-doctors-schedules"><h4><CalendarClock size={19} aria-hidden="true" /> Lịch làm việc trong tuần</h4>
            ...
            {!scheduleLoading && schedules?.length === 0 && <p className="reception-doctors-schedule-empty">Chưa có ca làm việc nào được API trả về.</p>}
          ...
          <p className="reception-doctors-note"><ShieldCheck size={17} aria-hidden="true" /> *API chưa cung cấp đơn vị tiền tệ cho phí khám. Lịch làm việc không xác nhận khung giờ còn trống; hãy kiểm tra khi đặt lịch.</p>
