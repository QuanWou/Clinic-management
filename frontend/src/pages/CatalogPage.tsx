...
export default function CatalogPage({ role }: { role: ClinicRole }) {
  ...
  if (!integrations.adminCatalog) return <>
    ...
    <Alert tone="info">Danh mục hiện chưa khả dụng.</Alert>
  </>;
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([serviceRequest, medicineRequest, getSpecialties()]).then(([serviceResult, medicineResult, specialtyResult]) => {
      ...
      if (serviceResult.status === 'fulfilled' && Array.isArray(serviceResult.value)) {
        ...
        setServices(null);
        failures.services = serviceResult.status === 'rejected' ? message(serviceResult.reason) : 'Danh sách dịch vụ không hợp lệ.';
      }
      if (medicineResult.status === 'fulfilled' && Array.isArray(medicineResult.value)) {
        ...
        setMedicines(null);
        failures.medicines = medicineResult.status === 'rejected' ? message(medicineResult.reason) : 'Danh sách thuốc không hợp lệ.';
      }
      if (specialtyResult.status === 'fulfilled' && Array.isArray(specialtyResult.value)) {
        setSpecialties(specialtyResult.value);
      } else {
        setSpecialties(null);
        failures.specialties = specialtyResult.status === 'rejected' ? message(specialtyResult.reason) : 'Danh mục chuyên khoa không hợp lệ.';
      }
      setErrors(failures);
  ...
  return <div className="catalog-workspace">
    ...
    <section className="catalog-hero" aria-label="Giới thiệu danh mục">
      <div className="catalog-hero-copy"><span className="catalog-hero-kicker"><ShieldCheck size={15} /> {admin ? 'DANH MỤC QUẢN TRỊ' : 'DANH MỤC THAM KHẢO'}</span>
        ...
        <span className="catalog-hero-chip"><CheckCircle2 size={15} /> {admin ? 'Danh sách dành cho quản trị viên' : 'Chỉ mục đang hoạt động'}</span></div>
      <span className="catalog-hero-icon" aria-hidden="true"><BookOpen size={72} strokeWidth={1.4} /></span>
    </section>

    {loading && <p role="status" className="catalog-loading">Đang tải danh mục...</p>}
    ...
    <section className="catalog-metrics" aria-label="Thống kê danh mục thực tế">
      <article className="catalog-metric"><span className="catalog-metric-icon"><ClipboardList size={21} /></span><strong>{services == null ? '—' : activeServices.length.toLocaleString('vi-VN')}</strong><h3>Dịch vụ đang hoạt động</h3><p>Trong danh mục hiện tại</p></article>
      ...
      <article className="catalog-metric catalog-metric-purple"><span className="catalog-metric-icon"><Stethoscope size={21} /></span><strong>{specialties == null ? '—' : specialties.length.toLocaleString('vi-VN')}</strong><h3>Chuyên khoa</h3><p>Trong danh mục hiện tại</p></article>
      {admin && <article className="catalog-metric catalog-metric-orange"><span className="catalog-metric-icon"><CircleAlert size={21} /></span><strong>{services == null || medicines == null ? '—' : inactiveCount.toLocaleString('vi-VN')}</strong><h3>Mục ngừng hoạt động</h3><p>Dịch vụ và thuốc</p></article>}
    </section>
    ...
    <section className="panel catalog-directory" aria-label="Tra cứu danh mục">
      <div className="catalog-section-heading"><div><span className="catalog-section-kicker">DỮ LIỆU THỰC</span><h3>Tra cứu danh mục</h3>
        <p>Chọn nhóm để tìm kiếm và xem thông tin chi tiết.</p></div><span className="catalog-source">Danh mục phòng khám</span></div>
      ...
      {entries !== null && <>
        ...
        {filtered.length === 0 && <p className="catalog-empty" role="status">{entries.length ? 'Không có mục phù hợp với điều kiện tìm kiếm.' : 'Chưa có dữ liệu cho nhóm này.'}</p>}
    ...
    {(selectedService || selectedMedicine || selectedSpecialty) && <section className="panel catalog-detail" aria-label="Chi tiết danh mục">
      ...
      {selectedService && <div className="catalog-price-section"><h4><Clock3 size={18} /> Giá đang hiệu lực</h4>
        ...
        {price && <div className="catalog-price-current"><div><span>Đơn giá hiện hành</span><strong>{formatCurrency(price.amount, price.currency)}</strong></div>
        ...
        {priceHistory?.length === 0 && <p>Chưa có lịch sử giá.</p>}
        {priceHistory?.map((entry) => <div key={entry.id} className="catalog-price-history"><span><strong>{formatCurrency(entry.amount, entry.currency)}</strong><small>Phiên bản: {entry.id}</small></span>
